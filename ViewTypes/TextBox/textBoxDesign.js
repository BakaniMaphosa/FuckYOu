...
import { initTableSettings } from "/Components/ContentTypes/tableSettings/TableSettings.js";

async function openTableSettingsModal() {
    let windowLevelOverlay = document.getElementById("WindowLevel");
    if (windowLevelOverlay) {
        // Clear existing content in WindowLevel
        windowLevelOverlay.innerHTML = ''; 
    } else {
        // Create WindowLevel if it doesn't exist
        windowLevelOverlay = document.createElement("div");
        windowLevelOverlay.id = "WindowLevel";
        document.body.appendChild(windowLevelOverlay);
    }

    // Apply FORCE overlay behaviour styles to WindowLevel
    Object.assign(windowLevelOverlay.style, {
        position: "fixed",
        inset: "0",
        zIndex: "999999",
        background: "rgba(77, 77, 77, 0.35)", // visible
        display: "flex",
    ...
    try {
        const response = await fetch('/Components/ContentTypes/tableSettings/TableSettings.html');
        windowLevelOverlay.innerHTML = await response.text();
        initTableSettings(windowLevelOverlay);
    } catch (error) {
        console.error('Error loading table settings:', error);
    }
    
    windowLevelOverlay.addEventListener("click", (e) => {
        // Only close if the click is directly on the overlay, not on the modal content
        if (e.target === windowLevelOverlay) {
            windowLevelOverlay.remove();
        }
    });
}
...
function insertInteractiveBox(targetZone, contentType = 'section') {
  ...
  box.style.cssText = `
    ...
    overflow: hidden;
  `;
  box.dataset.contentType = contentType;
...
export function setupEditorContextMenu(editor, contextMenu, viewRegistry = {}) {
  ...
  contextMenu.addEventListener("click", async (e) => {
    ...
    if (action === "insert-Table" && lastClickZone) {
      const box = insertInteractiveBox(lastClickZone, 'table');
      createTable(box);
      contextMenu.style.display = "none";
      return;
    }
